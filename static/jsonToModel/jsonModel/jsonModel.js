...
$(function() {
    ...
    let language = SFParserLanguage.Unknown;
    ...
    $('.make_model_btn').click(function (e) { 
        ...
        if (json_str.length > 0) {
            if (language == SFParserLanguage.Unknown) {
                alert('未知语言类型');
                return;
            }
            let json_parser = new SFJsonParser(language);
    ...
    function commitRecord(language_type) {
        let language = SFParserLanguage.desc(language_type)
        $.ajax({
            type: "post",
            url: "/jsonModel/",
