if (!-[1, ]) {
    ...
    function setBaseParam(options, callback) {
        ...
        options.m = parseInt(options.w / 300 * options.m);
        window.params = '&el=' + options.el + '&m=' + options.m + '&w=' + options.w;
        callback()
    }
    ...
    function createPic(type) {
        ...
        timer = setTimeout(function() {
            ...
            if (type) {
                ...
                var apiurl = $('#apiUrl').attr('title') || "https://xiaofu666.github.io";
            ...
            uri = '/api.php?' + colorParam.substr(1, colorParam.length) + '&' + uripart;
    ...
    $('#diy_div,#logotypes,#tabset>a:eq(2),#fnblock>div:eq(2),#tabset>a:eq(3),#fnblock>div:eq(3)').hide();
    $('.fnrow').addClass('mrow');
    ...
    $('#card_n,#card_tel,#card_phone,#car_note,#card_org,#card_til,#card_mail,#card_adr').keyup(function() {
        createPic('card')
    });
    ...
    $('#margin').change(function() {
        setBaseParam({
            m: parseInt($(this).val()),
            el: $("#level").val(),
            w: parseInt($("#size").val()) || 300
        }, createPic)
    });
    $('#level').change(function() {
        setBaseParam({
            m: parseInt($("#margin").val()),
            el: $(this).val(),
            w: parseInt($("#size").val()) || 300
        }, createPic)
    });
